classdef App < matlab.apps.AppBase
  ...
  function TreatmentOptions(app)
    ...
    if ~isempty(app.CurrentDiseaseEnglish)
        % Get the treatment options for the current disease
        options = app.getTreatmentOptions(app.CurrentDiseaseEnglish);

        % Display the options in a message box
        if ~isempty(options)
            uialert(app.DiseaseDetectionUIFigure, options, 'Treatment Options', 'Icon', 'info');
        else
            uialert(app.DiseaseDetectionUIFigure, 'No treatment options available for this disease.', 'Treatment Options', 'Icon', 'info');
        end
    else
  ...
  function options = getTreatmentOptions(~, disease)
        treatmentOptions = containers.Map(...
                 {'Pepper__bell___Bacterial_spot', ...
                 'Pepper__bell___healthy', ...
                 'Potato___Early_blight', ...
                 'Potato___healthy', ...
        ...
        if isKey(treatmentOptions, disease)
                    options = treatmentOptions(disease);
                else
                    options = 'No treatment options available.';
        end
